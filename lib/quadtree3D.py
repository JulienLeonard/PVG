...
class Quad3D:
    def __init__(self,bbox):
        ...
        self.msubquads = []
        self.mshapemap = {}
        self.mmaxshapenumber = 100

    def descr(self):
        (xmin,ymin,zmin,xmax,ymax,zmax) = self.mbbox.coords()
    ...
    def add(self,shape,push):
        ...
        if not self.intersect(bbox):
            self.mbbox = bbunion3D(self.mbbox,bbox)
    ...
    def split(self):
        newsubquads  = [Quad3D(bbox) for bbox in self.mbbox.split8()]
        ...
        if not self.intersect(newbbox):
            ...
            for push in self.mshapemap:
                for oshape in self.mshapemap[push]:
                    if Shape3D.intersect(newshape,oshape):
                        return True
                return False
...
class QuadTree3D:
    def __init__(self,bbox0=None):
        if bbox0 == None:
            bbox0 = BBox3D(-1000.0,-1000.0,-1000.0,1000.0,1000.0,1000.0)
        self.mrootquad = Quad3D(bbox0)
        self.mpush = 0
    ...
    def colliding(self,newshape):
        ...
        return [shape for shape in shapes if Shape3D.intersect(shape,newshape)]
