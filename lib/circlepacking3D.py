...
def nodes03D():
    ps = Circle().samples(4)
    return [SphereNode().coords((p.x(),p.y(),p.z(),0.5*sqrt(3.0))) for p in ps]
...
def sphere2nodetriplet(sphere):
    ...
    return [SphereNode().coords((p.x(),p.y(),p.z(),sphere.r()/2.0*sqrt(3.0))) for p in ps]


def circlepackingall3D(seeds,quadtree,radiusff,nitermax,minsize=0.0001):
    return CirclePacking3D().radiusf(radiusff).compute(seeds,quadtree,nitermax,minsize).spheres()

def circlepackingalladj3D(seeds,quadtree,radiusff,nitermax):
    return CirclePacking3D().radiusf(radiusff).compute(seeds,quadtree,nitermax)
        ...
        while len(front):
            ...
            if not quadtree.iscolliding(newsphere) and newsphere.radius() > minsize:
                ...
                for side in self.mnextsidesf():
                    front = self.minsertnewf(front,Seed3D3().seeddef([s1,s2,newsphere,side]))
